...
// TestChatFormatPassthrough verifies the cases where a format must reach the
// one completion call verbatim with nothing ahead of it: think=false on a
// builtin thinking parser (there is no thinking to close, so no closing
// strings go with the format and it binds from the first token), a JSON null
// format with think=true (null is not a format), and think=false against a
// model that emits thinking anyway. See
...
func TestChatFormatPassthrough(t *testing.T) {
	...
	cases := []struct {
		...
		// think=false: the format applies from the first token and must
		// reach the runner verbatim, with no closing strings.
		{
		...
		// true it reaches the runner as sent and constrains nothing. The
		// fork's two-pass flow once treated RawMessage("null") as a format
		// and ran a whole second pass to apply nothing (P0-7).
		{
			"gemma4-null-thinktrue", "gemma4", json.RawMessage(`null`), true,
			`{"answer":"42"}`,
		},
		// think:false + marker-wrapped thinking in the output:
		// Init(think:false) makes gemma4 discard channel events, so no
		// thinking surfaces and the one call still carries the format.
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.think && requests[0].ThinkingClose != nil {
				t.Errorf("think=false sent closing strings %q: the format would wait for a close that never comes", requests[0].ThinkingClose)
			}
		})
	}
}
...
// TestChatThinkFormatLengthNoContinuation verifies that a thinking response
// that runs out of num_predict before its thinking closes ends honestly as a
// length-limited result with empty content, in one completion call.

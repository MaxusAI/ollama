...
> **HISTORICAL — describes an approach that was never shipped. Do not implement from this.**
>
> This is the original implementation plan, written against Ollama's **Go-native
> inference runner**. Upstream has since deleted that entire path (`runner/ollamarunner/`,
> `model/model.go`, `model/models/gemma4/`), so none of the files below exist anymore and
> the `todos:` in the frontmatter above will never be actioned.
>
> **What actually shipped is different in both mechanism and numbers.** The feature landed
> via PR #2 (`d06138a9`) by passing `--image-min-tokens` / `--image-max-tokens` through to
> `llama-server`, touching only `api/types.go` and `llm/llama_server.go` — see
> `docs/maxusai/gemma4-budget-image.md` for the deployed configuration.
>
> | | this plan | shipped |
> | --- | --- | --- |
> | layer | Go runner (`ollamarunner`) | `llama-server` flags (C++ `mtmd`) |
> | defaults | 70 / 560 | **40 / 1120** |
> | ladder snap | `{70,140,280,560,1120}` | none — values passed through |
>
> Kept for the reasoning it records: the Google visual-token ladder, why budget changes
> must force a scheduler reload, and the option-naming rationale. Treat every file path,
> default, and acceptance criterion below as a description of the past.

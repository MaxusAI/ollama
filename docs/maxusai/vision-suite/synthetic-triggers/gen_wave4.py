...
# plus the strongest wave-1 periodics at 2048x2048 as positive controls for the clip path
ys, xs = np.mgrid[0:2048, 0:2048]
for p in (28, 56):
    save(f"checker_p{p}_2048x2048.png", np.where((((xs // p) + (ys // p)) % 2 == 1)[..., None], 255, 0).repeat(3, axis=2))
save("stripes_v_p56_2048x2048.png", np.where((((xs // 56) % 2 == 1))[..., None], 255, 0).repeat(3, axis=2))

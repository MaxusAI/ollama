...
# Override NUM_PREDICT to pin both modes to one value (not recommended).
...
for m in $MODELS; do
  ...
  for think in $THINK_MODES; do
    ...
    if [ "$think" = "on" ]; then np="${NUM_PREDICT:-16000}"; else np="${NUM_PREDICT:-4000}"; fi
    pmode=$( (pmset -g 2>/dev/null || true) | awk '/powermode/{print $2}')
    echo "##### MODEL $m think=$think tag=$tag $(date +%T) powermode=${pmode:-n/a} num_predict=$np"
    ...
    ENDPOINT="${ENDPOINT:-chat}" THINK="$think" NUM_PREDICT="$np" \

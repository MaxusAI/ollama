...
> **This lineage carries `preflight/` only.** `release/0.32.1-dynres` has no
> `docs/maxusai/vision-suite/` beyond this directory, so `vision_suite.py`,
> `measure.py`, `extbench.py` and the scene generators are **not here** — they are
> on `main`. Consequences, all deliberate:
>
> * `--quality` reports **SKIP** with a reason rather than failing. Every other
>   check is unaffected; the token ladder, payload proof, pinned-budget probe and
>   think+format arm need nothing from the suite.
> * `go test ./llm/ -run TestImageTokensForSize` does not exist on this lineage
>   either, so the no-GPU section below has one gate, not two.
> * `measure.py` is referenced below as the *history* of why the subtrahend is
>   calibrated the way it is, not as a file you can open here.

> **Getting the subtrahend right takes two corrections, not one.** Both are in
> `probes.Ollama.image_prefix()`.
>
...
> which agrees with the pinned `expect_tokens` (and, on `main`, with
> `TestImageTokensForSize`).

## Relationship to the no-GPU gate

One cheap gate runs before any model loads:

```bash
python3 test_verdicts.py                     # the harness's own verdict logic
```

`main` has a second — `go test ./llm/ -run TestImageTokensForSize`, which pins the
sizing formulas. **That test is not on this lineage**, so the formulas are covered
here only by the live ladder against a real server.

`test_verdicts.py` pins *this harness*: that a flat ladder diagnoses an unpatched payload for a `dynamic` arch
...
base, text_only, _ = c.image_prefix(model)   # NOT text_baseline() -- see above

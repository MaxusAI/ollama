...
This is not architecture specific, and not in a hand-wavy way. `ggml_cuda_mmq_get_J_max` never reads `cc` before the `ret -= ret % 8` step, so `ne11 == 1` reduces to zero before the architecture is consulted at all.

`get_J_max` and `ggml_cuda_mmq_get_config` are pure functions of `(type, J, fallback, cc)` — no `ggml_cuda_info()`, no device queries — so this can be evaluated on the host with no GPU. Compiling the real `mmq.cuh` and sweeping every `cc` a device could plausibly report (NVIDIA 500..1300, AMD `GGML_CUDA_CC_OFFSET_AMD + gfx` for gfx 900..1300), six quant types, both `fallback` values:

```
swept 5784 (cc x type x fallback) at ne11 = 1
returned ZERO padding: 5784  (nvidia 972, amd 4812)
any cc that gives non-zero padding at ne11=1? NO - none
```

There is no `cc` that avoids it. Per family, the degenerate range extends to `ne11 = 7` on NVIDIA, Vega and RDNA2, and to `ne11 = 15` on RDNA3/3.5/4 and CDNA2/3, where no valid config exists at `J = 8` either and the loop steps down to zero.

To be clear about what this does and does not show: it is a unit evaluation of the sizing function, not of the dispatch. It does not execute `ggml_cuda_info().devices[...].cc`, `ggml_cuda_should_use_mmq`, or the kernel.

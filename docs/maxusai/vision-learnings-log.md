...
- **Enforced by** — `ocrbench_table.py` (H7 generator) computes the discordant
  counts and an **exact** McNemar, and prints `resolved: no` rather than an
  ordering when a pair does not clear it. Exact, not chi-square: at 3-vs-1 the
  uncorrected approximation gives p≈0.317 against the true 0.625, and 0.317 is
  the kind of number that gets reported as "trending". A second renderer was
  written for this in parallel and dropped rather than shipped — SPEC H1's "a
  second one is the defect" applies to summarizers, not just runners.

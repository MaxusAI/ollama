...
Run 2026-09-19 on the Apple Silicon host `10.8.0.3` (M5 Max, 128 GB, macOS 26.6.2),
build **`0.34.0-maxusai-8a7ba949`** — the deployed one, equivalently
`0.34.1-dynres-0-g8a7ba94` (ADR 0032, 2026-09-19 amendment). This PR's protocol
exactly: rows 0–200, `THINK=false`, `NUM_CTX=8192`, `NUM_PREDICT=512`, `SLEEP=1`,
`/api/generate`, one model at a time, **cold server per run, two runs per arm**.

**Which artifacts.** `store_audit.py --store ~/.ollama/models-mlx gemma4` reports that
**all seven** quantised gemma4 MLX tags on this host have diverged from the registry,
so every arm is named by its local manifest digest and run against a preserved tag
that a pull cannot overwrite:

| arm | tag measured | manifest digest | vs the registry |
|---|---|---|---|
| nvfp4 / nvfp4 tower | `gemma4:31b-nvfp4-tower-nvfp4` | `637cc0ff1570` | 194/1248 layers differ (vision nvfp4 → bf16, 3.52×) |
| mxfp8 / 8-bit tower | `gemma4:31b-mxfp8-tower-mxfp8` | `1434769c1561` | **579/1247** differ — language 411, vision 162 |
| bf16 | `gemma4:31b-mlx-bf16-tower-bf16` | `fb3f25b3bc8d` | 4/1247 — `tokenizer_config.json` and two `draft/` configs |

`637cc0ff1570` is **the same artifact as this ladder's `nvfp4 / nvfp4 tower` row on
mlx-cuda**, so those two rows are one checkpoint on two backends. The other two are
not: **the mxfp8 row here is a different checkpoint from the mlx-cuda mxfp8 row**,
which used the registry's bf16-tower copy — 579 layers apart. It is a local artifact,
not a backend comparison, and must not be read across.

<!-- GENERATED by vision-suite/summarize_extbench.py; re-render, do not hand-edit. -->

| model | scored | errors | empty | correct | accuracy | think | endpoint |
|---|---|---|---|---|---|---|---|
| `gemma4:31b-nvfp4-tower-nvfp4` | 200 | 0 | 0 | 175 | **0.875** | false | generate |
| `gemma4:31b-mxfp8-tower-mxfp8` | 200 | 0 | 0 | 173 | **0.865** | false | generate |
| `gemma4:31b-mlx-bf16-tower-bf16` | 200 | 0 | 0 | 169 | **0.845** | false | generate |

ocrbench — `echo840/OCRBench` [test], rows 0..200.

host: http://127.0.0.1:11436 · build: 0.34.0-maxusai-8a7ba949

| pair | both ✓ | both ✗ | A only | B only | McNemar exact p |
|---|---|---|---|---|---|
| ocr31b_metal_nvfp4tower_r1 vs ocr31b_metal_mxfp8local_r1 | 171 | 23 | 4 | 2 | 0.688 |
| ocr31b_metal_nvfp4tower_r1 vs ocr31b_metal_bf16_r1 | 169 | 25 | 6 | 0 | 0.031 |
| ocr31b_metal_mxfp8local_r1 vs ocr31b_metal_bf16_r1 | 168 | 26 | 5 | 1 | 0.219 |

| arm | runs | accuracies | items that changed verdict |
|---|---|---|---|
| nvfp4 / nvfp4 tower | 2 | 0.875, 0.875 | 0 |
| mxfp8 / 8-bit tower (local) | 2 | 0.865, 0.865 | 0 |
| bf16 | 2 | 0.845, 0.845 | 0 |

| arm | accuracy | ±1 s.e. | mean s/item | median | mean prompt_eval |
|---|---|---|---|---|---|
| nvfp4 / nvfp4 tower | 0.875 | 0.023 | 5.3 | 4.5 | 1115 |
| mxfp8 / 8-bit tower (local) | 0.865 | 0.024 | 7.5 | 7.0 | 1115 |
| bf16 | 0.845 | 0.026 | 4.9 | 5.2 | 1115 |

### Reading it

**Accuracy falls monotonically as precision rises** — nvfp4 0.875, mxfp8 0.865, bf16
0.845 — and every arm reproduced its own accuracy exactly across two cold runs, with
**zero items changing verdict**. The mlx-cuda rows above show the same ordering
independently (nvfp4 0.860 > mxfp8 0.845).

**One pair clears the paired test: nvfp4 beats bf16, 6 items to 0, exact McNemar
p = 0.031.** That is the only significant pair in this section, and it should be read
with two caveats. Three pairwise tests were run, so a multiplicity correction
(Bonferroni: 0.031 × 3 ≈ 0.09) puts it back above 0.05. And rows 0–200 are OCRBench's
text-recognition half, not the benchmark.

**Timing is not ordered by precision either**: bf16 is the fastest per item (4.9 s)
and mxfp8 the slowest (7.5 s), with nvfp4 between (5.3 s).

### The kernel A/B that started this

Same host, same checkpoint (`637cc0ff1570`), one archived binary per arm, **all 1000
items** rather than the 200-row slice:

| arm | correct / scored | accuracy |
|---|---|---|
| `0.34.0-maxusai-8a7ba949` — MLX `d9add9d1`, #3912 fixed | 835 / 1000 | **0.835** |
| `0.33.2-maxusai-2b95b4a5` — MLX `c793734e`, the defect | 833 / 1000 | **0.833** |

Paired: b = 9, c = 7, **p = 0.804**. The MLX #3912 kernel defect is undetectable end
to end, which is what ADR 0037 rests on. Arms `ocr31b_metal_ab_{fixed,defect}`.

### The arms are committed

Every arm cited above is under `vision-suite/bench-runs/ocrbench/`, with a
`.gitignore` negation, so these tables re-render from the repo alone:

```
summarize_extbench.py --dir bench-runs/ocrbench --repeats --paired --timing ocrbench \
  "nvfp4 / nvfp4 tower=ocr31b_metal_nvfp4tower_r1,ocr31b_metal_nvfp4tower_r2" \
  "mxfp8 / 8-bit tower (local)=ocr31b_metal_mxfp8local_r1,ocr31b_metal_mxfp8local_r2" \
  "bf16=ocr31b_metal_bf16_r1,ocr31b_metal_bf16_r2"
```

Dataset **rows** are deliberately not committed: `--categories` reads extbench's local
row cache, so a category breakdown needs a fetch on the host rendering it, and this
repo does not redistribute the benchmark's content.

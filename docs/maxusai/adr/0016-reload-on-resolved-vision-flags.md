...
- **Status:** accepted, implemented + verified 2026-08-10 on fork `main`
  lineage. Refines invariant B3 of
...
every arch, but carrying *gemma4's* defaults ([ADR 0008](0008-gemma4-budget-fill-restores-1120.md)),
because `api.DefaultOptions()` has one set of values for all models. Arches
with their own native bounds therefore treat the gemma4 defaults as "caller
left this alone" and substitute their own ([ADR 0001](0001-nemotron-vision-dynamic-resolution.md)
for nemotron; the sentinel is the `api.Default*` constants, so its literal
value moves with the gemma4 default — 70/1120 today).
...
- Follow-up: none required. If the `*int` change is ever revisited, this
  normalization stays — it is orthogonal.

...
(`mlxrunner/xgrammar/structured_parity_test.go`, deleted with the package) drove both engines live over a
...
`mlxrunner/xgrammar/engine_behaviour_test.go`, which fails when an xgrammar bump changes them:

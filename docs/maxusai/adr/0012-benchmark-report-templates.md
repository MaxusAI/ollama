...
|---|---|---|---|
| **T1 — Campaign matrix** | two tables: grounding+extraction, then OCR tiers + multi + Think tok (token_split.py's gate-proven reasoning-token count; `—` when unsplit) + throughput + latency; one row per model, `Engine` column, MLX bolded | full N-model campaigns on one host/power state | `vision-suite/summarize_engine_compare.py` (paired with `run_engine_compare.sh`; `--prefix` renders a TAG_PREFIX campaign namespace, e.g. `--prefix cudafull1_`) |

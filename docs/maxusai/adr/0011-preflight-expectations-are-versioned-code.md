...
- **Related:** [`004-llama-cpp-gemma4-budget-fill.patch`](../../../llama/compat/004-llama-cpp-gemma4-budget-fill.patch)
  and [`005-llama-cpp-dynres-pinned-overshoot.patch`](../../../llama/compat/005-llama-cpp-dynres-pinned-overshoot.patch)
  (both changed expected values inside one week; their behaviour on this lineage is
  [SPEC §2.1 and §2.2](../spec/vision-image-token-budgets.md)),
  [ADR 0003](0003-vision-image-token-budget-policy.md) (budgets are per-arch)

> **Backported from `main` — adapted.** This lineage is `release/0.32.1-dynres`
> (ollama 0.32.1 base, llama.cpp **b9888**), so two things differ from the version
> of this ADR on `main`:
>
> * **ADR 0006** (never merge this lineage into `main`) and **ADR 0008** (gemma4
>   budget-fill) are `main`-only documents. Their conclusions still govern — 0006
>   is *why* this file arrived by cherry-pick — but the links are omitted rather
>   than left dangling. 004/005 behaviour here is specified in
>   [SPEC §2.1/§2.2](../spec/vision-image-token-budgets.md).
> * **The vision suite is not on this branch.** Only `preflight/` was taken, so
>   the `--quality` arm skips and the `measure.py` history below is context for
>   why the subtrahend is calibrated, not a file you can open here.
...
neither is present on this lineage.** What was missing was a **pass/fail layer with
expectations as data**: `measure.py`'s own README notes it *reports rather than
asserts*.
...
|---|---|---|
| [004](../../../llama/compat/004-llama-cpp-gemma4-budget-fill.patch) | gemma4 budget-fill sizing (ADR 0008 on `main`; [SPEC §2.1](../spec/vision-image-token-budgets.md) here) | size-scaling ladder → **flat 1102 at every resolution** |
...
5. **An unknown `(platform, version)` combination is a hard error (exit 2), never a
   ...
   The AMD gate (`docs/maxusai/amd-upgrade-gate.md`, on `main`) blocks the 0.32.5 base and its b10091
...
- The ROCm and Apple Silicon profiles ship **declared but unmeasured**. Those hosts
  ...
  over.
- Per ADR 0006 rule 1 (on `main`), the

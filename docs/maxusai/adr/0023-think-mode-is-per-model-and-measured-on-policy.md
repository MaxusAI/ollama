...
- **⚠ Lineage note — none of this is measured on this lineage.** Backported from `main`
  (`b237ec9e`) so that this branch does not carry only the superseded half of the argument.
  Its evidence is **Apple Silicon on payload b10353**. This lineage is **gfx1151 / ROCm on
  b9888** — a different host *and* a payload the AMD upgrade gate deliberately keeps us
  behind (`docs/maxusai/amd-upgrade-gate.md`, on `main`). ADR 0023 itself states the hosts
...
- **Related:** [ADR 0022](0022-thinking-is-off-for-vision-work.md) (superseded by this),
  [ADR 0011](0011-preflight-expectations-are-versioned-code.md) (`min_num_predict`).
  On `main` only: `docs/maxusai/runaway-reasoning-under-think.md` (the sampling defect and
  its evidence), ADR 0005 (per-model runtime configuration precedent), ADR 0012 (±0.01
  noise floor).
...
Full evidence in `runaway-reasoning-under-think.md` (on `main`);

	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			withMLXThread(t, func(t *mlxthreadtest.T) {
				...
				// A stored global scale is the checkpoint's own multiplier
				// (ADR 0039), so the factors ARE the stored form. Upstream's
				// fixture multiplies by Nvfp4MaxProduct because upstream
				// stores MLX's form.
				...
				// The comparison above cannot see a representation error:
				// both sides route through globalScaleFactor, so they agree
				// whether it is the identity or divides by Nvfp4MaxProduct.
				// Apply the factor directly instead. The v0.34.4 fold merged
				// a globalScaleFactor that divided by 2688 under this fork's
				// m-valued scales, cleanly and without a conflict; this is
				// the check that fails when that happens.
				directScale := func(x *Array, factors []float32) *Array {
					if factors == nil {
						return x
					}
					return Mul(x.AsType(DTypeFloat32), storedScale(factors)).AsType(x.DType())
				}
				direct := SwiGLU(directScale(gate, tt.gateScale), directScale(up, tt.upScale)).AsType(DTypeFloat32)
				Eval(direct)
				for i, d := range direct.Floats() {
					if tol := 0.02*math.Abs(float64(d)) + 1e-2; math.Abs(float64(gotValues[i]-d)) > tol {
						t.Fatalf("SwiGLUScaled()[%d] = %v, want %v from the stored scale applied as the multiplier (ADR 0039)", i, gotValues[i], d)
					}
				}
			})
		})

		...
		t.Run(tt.name, func(t *testing.T) {
			withMLXThread(t, func(t *mlxthreadtest.T) {
				...
				storedScale := func(factors []float32) *Array {
					...
					for i := range factors {
						values[i] = factors[i] // stored as the checkpoint's own multiplier (ADR 0039)
					}
					if len(values) == 1 {
...
func benchmarkSwiGLUScaling(b *testing.B, fn func(gate, gateScale, up, upScale *Array) *Array) {
	...
	if err := thread.Do(b.Context(), func() error {
		...
		gateScale := FromValue(float32(0.75))
		upScale := FromValue(float32(1.25))
...
// TestSwiGLUScaledAppliesTheStoredScale checks SwiGLUScaled against a reference
// computed without the scale helpers. TestSwiGLUScaledMatchesSeparateScaling
// compares two paths through the same helpers, so it passes under any
// consistent representation; this pins the fork's: a stored global scale is the
// checkpoint's own multiplier and is applied as is (ADR 0039). A helper that
// divides by Nvfp4MaxProduct, as upstream's does, is 2688x off here.
func TestSwiGLUScaledAppliesTheStoredScale(t *testing.T) {
	gateValues := []float32{-3.25, -1.5, -0.25, 0.5, 1.75, 3, 4.5, 6}
	upValues := []float32{2.5, -2, 1.25, -0.75, 0.125, 1.5, -3.5, 5}
	const gateScale, upScale = 0.75, 1.25
	var got []float32
	withMLXThread(t, func(t *mlxthreadtest.T) {
		EnableCompile()
		gate := FromValues(gateValues, 2, 4).AsType(DTypeBFloat16)
		up := FromValues(upValues, 2, 4).AsType(DTypeBFloat16)
		out := SwiGLUScaled(gate, FromValue(float32(gateScale)), up, FromValue(float32(upScale))).AsType(DTypeFloat32)
		Eval(out)
		got = out.Floats()
	})
	if len(got) != len(gateValues) {
		t.Fatalf("got %d values, want %d", len(got), len(gateValues))
	}
	for i := range gateValues {
		g, u := float64(gateValues[i]*gateScale), float64(upValues[i]*upScale)
		want := g / (1 + math.Exp(-g)) * u
		// bf16 keeps 8 significant bits and the kernel rounds three times.
		if delta := math.Abs(float64(got[i]) - want); delta > 0.02*math.Abs(want)+1e-3 {
			t.Errorf("SwiGLUScaled()[%d] = %v, want %v (delta %v)", i, got[i], want, delta)
		}
	}
}

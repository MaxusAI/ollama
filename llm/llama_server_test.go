...
func TestApplyArchServerEnvs(t *testing.T) {
	tests := []struct {
		...
		{
			// =f16 is the documented way to reproduce stock fp16-accumulate
			// behavior; the arch default must not clobber an operator value.
...
func TestResolvedImageTokenBudget(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Normalize must agree with the resolver and leave the launch
			// unchanged: the normalized bounds re-resolve to themselves.
			r := tc.opts.Runner
			...
			if tc.wantDerived {
				gotMin, gotMax, _ := ResolvedImageTokenBudget(tc.arch, api.Options{Runner: r})
				if gotMin != tc.wantMin || gotMax != tc.wantMax {
					t.Errorf("normalize is not idempotent for %q: %d/%d", tc.arch, gotMin, gotMax)
				}
			}
		})
	}

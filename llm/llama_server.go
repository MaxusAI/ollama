...
func (s *llamaServerRunner) runCompletionPhase(ctx context.Context, req CompletionRequest, phase llamaServerCompletionPhase, fn func(CompletionResponse)) (result llamaServerPhaseResult, _ error) {
	lsReq := llamaServerCompletionRequest{
		...
		// Only the two-pass think+format flow (OLLAMA_FORMAT_TWO_PASS, ADR 0004)
		// sets this: its first pass is cancelled before a final response, so
		// every chunk must carry the metrics. Single pass never does.
	...
	if len(req.ThinkingClose) > 0 && (lsReq.Grammar != "" || lsReq.JsonSchema != nil) {
		if lsReq.Grammar == "" {
			...
			if err != nil {
				// upstream's block returns bare err; here it runs inside the
				// fork's runCompletionPhase, which returns (result, err).
				return result, err
			}
	...
	for scanner.Scan() {
		...
		default:
			...
			if tokenRepeat > 100 {
				...
				return result, fmt.Errorf("prediction aborted, token repeat limit reached")
			}

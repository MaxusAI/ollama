...
func TestCheckVisionPrefillBudget(t *testing.T) {
	bidi := func(length int) mediaItem {
		return mediaItem{length: length, item: &model.PreparedItem{Causal: false}}
	}
	causal := func(length int) mediaItem {
		return mediaItem{length: length, item: &model.PreparedItem{Causal: true}}
	}
...
func TestVisionPrefillBudgetBoundsTheAllocation(t *testing.T) {
	items := []mediaItem{{length: 4096, item: &model.PreparedItem{Causal: false}}}
...
func TestExtendChunkKeepsBidiBlocksInTheOpeningChunk(t *testing.T) {
	bidiItem := func(pos, length int) mediaItem {
		return mediaItem{pos: pos, length: length, item: &model.PreparedItem{Causal: false}}
	}
	causalItem := func(pos, length int) mediaItem {
		return mediaItem{pos: pos, length: length, item: &model.PreparedItem{Causal: true}}
	}

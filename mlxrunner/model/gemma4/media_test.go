...
func textSeg(tokens ...int32) model.Segment { return model.Segment{Tokens: tokens} }
func imgSeg(data []byte) model.Segment      { return model.Segment{Kind: "image", Data: data} }
...
func TestPrepareMediaWithBudgetSplicesExpansion(t *testing.T) {
	...
	prepared, err := m.PrepareMediaWithBudget(
		[]model.Segment{textSeg(1, 2), imgSeg(png), textSeg(3)}, 0, 70)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPrepareMediaWithBudgetSeparatesBudgets(t *testing.T) {
	...
	low, err := m.PrepareMediaWithBudget([]model.Segment{imgSeg(png)}, 0, 70)
	if err != nil {
		t.Fatal(err)
	}
	high, err := m.PrepareMediaWithBudget([]model.Segment{imgSeg(png)}, 0, 280)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPrepareMediaDefaultsToTheModelsOwnBudget(t *testing.T) {
	...
	unset, err := m.PrepareMedia([]model.Segment{imgSeg(png)})
	if err != nil {
		t.Fatal(err)
	}
	explicit, err := m.PrepareMediaWithBudget(
		[]model.Segment{imgSeg(png)}, api.DefaultImageMinTokens, api.DefaultImageMaxTokens)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPrepareMediaRejectsNonImage(t *testing.T) {
	m := visionTestModel()
	_, err := m.PrepareMedia([]model.Segment{{Kind: "audio", Data: []byte("not an image")}})
	if err == nil {

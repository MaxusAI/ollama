...
// PrepareMediaWithBudget implements model.MediaBudgetModel. glimmer keeps its own
...
func (m *Model) PrepareMediaWithBudget(segments []model.Segment, imageMinTokens, imageMaxTokens int) (*model.PreparedRequest, error) {
	_, maxTokens := model.ResolveImageBudget(imageMinTokens, imageMaxTokens, 0, maxImageTokens)

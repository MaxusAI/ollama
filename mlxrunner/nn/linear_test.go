		...
		t.Run(tt.name, func(t *testing.T) {
			mlxtest.Run(t, func(t *mlxtest.T) {
				...
				if tt.quantized {
					...
					if tt.withScales {
						// Stored as the checkpoint's own multipliers (ADR 0039).
						gateQuantized.GlobalScale = mlx.FromValue(float32(0.75))
						upQuantized.GlobalScale = mlx.FromValue(float32(1.25))
					}

...
func TestVisionGoldenParity(t *testing.T) {
	...
	// v0.34.2 dropped Root.Close, which was a no-op; the open is still the
	// availability check this skip depends on.
	if _, err := model.Open(modelName); err != nil {
	...
	// Drives the model.MediaBudgetModel path the runner actually uses, so this
	// golden comparison gates PrepareMediaWithBudget and EncodeMedia rather than
	// the superseded per-model vision entry points.
	mm, ok := r.Model.(model.MediaBudgetModel)
	if !ok {
		t.Fatalf("%T does not implement model.MediaBudgetModel", r.Model)
	}

	prepared, err := mm.PrepareMediaWithBudget(
		[]model.Segment{{Kind: "image", Data: gradientPNG(t, 480, 336)}}, 0, 70)
	if err != nil {
		t.Fatal(err)
	}

...
func mediaFoldPrompt(imgTokens int, payload string) ([]int32, []mediaItem) {
	...
	item := mediaItem{
		...
		// A causal item: begin() reads item.item.Causal (v0.34.1's non-causal
		// boundary rule) on every item, and this test is about fold identity,
		// not atomic expansions. openMedia never leaves item nil.
		item: &model.PreparedItem{Causal: true},
	}
